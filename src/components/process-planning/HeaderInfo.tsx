...
export function HeaderInfo() {
  ...
  return <>
    <div className="stepper" aria-label="工作流程">{steps.map((label,i)=><div className={`step ${i<5?"done":""} ${i===5?"active":""}`} key={label}><div className="step-num">{String(i+1).padStart(2,"0")}</div><div>{label}</div></div>)}<div className="head-tools"><button>⇩ 匯出報告</button><button>▣ 列印</button></div></div>
