...
function LibraryCard({icon,title,count,categories,warning,detail,onOpen}:{icon:string;title:string;count:number;categories:number;warning:number;detail:string;onOpen:()=>void}){return <article className="library-card"><span>{icon}</span><div><h3>{title}</h3><p>{detail}</p><div><b>{count}<small>筆資料</small></b><b>{categories}<small>分類</small></b><b className={warning?"warn":""}>{warning}<small>庫存示警</small></b></div></div><button onClick={onOpen}>開啟管理 →</button></article>}
