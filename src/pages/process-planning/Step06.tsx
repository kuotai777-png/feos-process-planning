...
export default function Step06(){const[selected,setSelected]=useState("採用方案 A");const[saved,setSaved]=useState(false);const save=()=>{setSaved(true);window.setTimeout(()=>setSaved(false),2200)};return <AppLayout><main className="page"><HeaderInfo/><Section index="一" title="流程方案分析" note="AI 依據證據與數據提出多種加工流程方案"><div className="plan-grid">{processPlans.map(p=><ProcessPlanCard plan={p} key={p.id}/>)}</div><div className="formula-note">ⓘ CP 值計算：綜合成本、品質、效率、設備適配率、風險等因素進行加權計算（滿分 100）</div></Section><div className="analysis-pair"><Section index="二" title="目前工廠設備分析"><EquipmentAnalysisCard/></Section><Section index="三" title="目前訂單分析"><OrderLoadChart/></Section></div><Section index="四" title="解決方案比較"><SolutionComparisonTable/></Section><Section index="五" title="人工決策"><HumanDecisionPanel selected={selected} onSelect={setSelected}/></Section></main><FooterAction onSave={save}/>{saved&&<div className="toast">✓ 草稿已儲存</div>}</AppLayout>}
